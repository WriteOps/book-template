...
# Note: Chapter images are now handled by scripts/download-images.lua
download_asset() {
    local url=$1
    local extension="${url##*.}"
...
# Initialize FILES variable with just the preface
FILES="meta/preface.md"
...
pandoc output/merged.md \
  ...
  --include-before-body=meta/title-copyright.tex \
  --filter ./node_modules/.bin/mermaid-filter \
  --lua-filter ./scripts/download-images.lua \
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${GREEN}📖 The PDF has been created at: ${BOLD}output/book.pdf${NC}\n"
